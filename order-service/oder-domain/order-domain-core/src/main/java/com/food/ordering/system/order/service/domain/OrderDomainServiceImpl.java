...
public class OrderDomainServiceImpl implements OrderDomainService {
    ...
    private void setOrderProductInformation(Order order, Restaurant restaurant) {
        order.getItems().forEach( orderItem-> restaurant.getProducts().forEach(restaurantProduct-> {
            Product curretProduct = orderItem.getProduct();
            if(curretProduct.equals(restaurantProduct)){
                curretProduct.updateWithConfirmedNameAndPrice(restaurantProduct.getName(), restaurantProduct.getPrice());
            }
        }));
    }
    ...
    public OrderCancelledEvent cancelOrderPayment(Order order, List<String> failureMessages) {
        ...
        log.info("Order with id {} is iniCancel.", order.getId().getValue());
        return new OrderCancelledEvent(order, ZonedDateTime.now(ZoneId.of(UTC)));
    }

    @Override
    public void canceledOrder(Order order, List<String> failureMessages) {
